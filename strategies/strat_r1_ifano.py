...
# marque un arrêt et ferme la pince, attends une demi seconde avant de reprendre le chemin
    set_pince_commande(robot, 1, "OUVERTURE")
    await robot.movement.async_angle_0(110, 9)
    set_pince_commande(robot, 1, "FERMETURE")
    time.sleep(0.5)
# fait marche arrière jusqu'à la case verte puis bleue avec un 90° vers la droite, marche avant, 90° vers la droite, 30°
# robot.movement.rotation_droite(115) #9.67 sec pour 2 tours sur lui même quasi parfait
    await robot.movement.async_angle_180(110, 7)
    await robot.movement.async_rotation_droite(115, 9.67 / 2 / 4)
    time.sleep(0.5)
    await robot.movement.async_angle_0(110, 5.5)
    time.sleep(0.5)
    await robot.movement.async_rotation_droite(115, 9.67 / 2 / 12)
    time.sleep(0.5)
# ouvre la pince et avance, la referme, fait une rotation, ouvre la pince et avance, la referme, fait marche arrièce
    set_pince_commande(robot, 3, "OUVERTURE")
    time.sleep(0.5)
    await robot.movement.async_angle_240(110, 3)
    set_pince_commande(robot, 3, "FERMETURE")
    time.sleep(0.5)
    await robot.movement.async_rotation_droite(115, 9.67 / 2 / 3)
    time.sleep(0.5)
    set_pince_commande(robot, 2, "OUVERTURE")
    time.sleep(0.5)
    await robot.movement.async_angle_120(110, 4)
    set_pince_commande(robot, 5, "FERMETURE")
    time.sleep(1.5)
    await robot.movement.angle_300(110, 2) #1.5
    # set_pince_commande(robot, 2, "STANDBY") 2 lignes plus tard
    await robot.movement.async_rotation_droite(115, 9.67 / 2 / 2)
    set_pince_commande(robot, 2, "STANDBY")
    set_pince_commande(robot, 1, "OUVERTURE")
    await robot.movement.async_angle_0(110, 1)
    set_pince_commande(robot, 4, "FERMETURE")
    time.sleep(1)
    await robot.movement.async_angle_180(110, 1) #0.5
# set_pince_commande(robot, 1, "STANDBY")
    set_pince_commande(robot, 1, "STANDBY")
    set_pince_commande(robot, 3, "OUVERTURE")
    await robot.movement.async_angle_240(110, 1)
    set_pince_commande(robot, 6, "FERMETURE")
    time.sleep(0.5)
    time.sleep(1)
    await robot.movement.async_angle_60(110, 0.5)
    set_pince_commande(robot, 3, "STANDBY")

# tourne de 60° à gauche et recule jusqu'au carré du coin puis tourne de 30° à gauche pour "reculer" et toucher la
    # zone de départ
    print("basing")
    await robot.movement.async_rotation_gauche(115, 9.67 / 2 / 6)
    await robot.movement.async_angle_180(110, 6)
    await robot.movement.async_rotation_gauche(115, 9.67 / 2 / 12)
    await robot.movement.async_angle_300(110, 3.5)
    print("basing done")
